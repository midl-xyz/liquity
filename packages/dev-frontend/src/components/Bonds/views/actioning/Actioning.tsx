        ...
        <>
          ...
          <SubLabel>
            <InfiniteEstimate estimate={bond.breakEvenAccrual}>
              {bond.breakEvenAccrual.prettify(6)} bLUSD
            </InfiniteEstimate>
          </SubLabel>
        </>
      )
        ...
        <>
          ...
          <SubLabel>
            <InfiniteEstimate estimate={bond.rebondAccrual}>
              {bond.rebondAccrual.prettify(6)} bLUSD
            </InfiniteEstimate>
          </SubLabel>
        </>
      )
    },
    {
      ...
      label: (
        <>
          ...
          <SubLabel style={{ fontWeight: 400 }}>{`${bond.accrued.prettify(6)} bLUSD`}</SubLabel>
        </>
      ),
      isEndOfLife: true,
      isMilestone: false
  ...
  return (
    <ReactModal onDismiss={handleDismiss}>
      ...
      <Grid gap="12px" columns={3} sx={{ my: 4, justifyItems: "center" }}>
        ...
        <Record lexicon={l.MARKET_VALUE} value={bond.marketValue.prettify(6)} type="BUSD" />

        {view === "CLAIMING" && (
          <Record lexicon={l.BOND_RETURN} value={bond.claimNowReturn.toFixed(2)} type="BUSD" />
        )}
      </Grid>
      <details>
