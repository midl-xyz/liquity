...
const TroveChangeDescription: React.FC<TroveAdjustmentDescriptionParams> = ({ params }) => (
  <ActionDescription>
    {params.depositCollateral && params.borrowLUSD ? (
      <>
        You will deposit <Amount>{params.depositCollateral.prettify(6)} BTC</Amount> and receive{" "}
        <Amount>
      ...
      <>
        ...
        and receive <Amount>{params.withdrawCollateral.prettify(6)} BTC</Amount>
      </>
    ) : params.depositCollateral && params.repayLUSD ? (
      <>
        You will deposit <Amount>{params.depositCollateral.prettify(6)} BTC</Amount> and pay{" "}
        <Amount>
          {params.repayLUSD.prettify()} {COIN}
        </Amount>
      </>
      ...
      <>
        You will receive <Amount>{params.withdrawCollateral.prettify(6)} BTC</Amount> and{" "}
        <Amount>
          {params.borrowLUSD.prettify()} {COIN}
        </Amount>
      </>
      ...
      <>
        You will deposit <Amount>{params.depositCollateral.prettify(6)} BTC</Amount>
      </>
    ) : params.withdrawCollateral ? (
      <>
        You will receive <Amount>{params.withdrawCollateral.prettify(6)} BTC</Amount>
      </>
    ) : params.borrowLUSD ? (
      <>
        You will receive{" "}
  ...
  if (depositCollateral.gt(accountBalance)) {
    return (
      <ErrorDescription>
        ...
        <Amount>{depositCollateral.sub(accountBalance).prettify(6)} BTC</Amount>.
      </ErrorDescription>
    );
  }
...
const validateTroveClosure = (
  ...
  if (numberOfTroves === 1) {
    return (
      <ErrorDescription>
        You're not allowed to close your Position when there are no other Troves in the system.
      </ErrorDescription>
    );
  }
  ...
  if (recoveryMode) {
    return (
      <ErrorDescription>
        You're not allowed to close your Position during recovery mode.
      </ErrorDescription>
    );
  }
  ...
  if (wouldTriggerRecoveryMode) {
    return (
      <ErrorDescription>
        You're not allowed to close a Position if it would cause the Total Collateralization Ratio to
