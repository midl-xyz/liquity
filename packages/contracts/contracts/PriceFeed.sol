...
import "./Interfaces/IBTCOracle.sol";
...
/*
 * PriceFeed for production deployment, to be connected to Main Oracle's live IBTCOracle:IBTCOracle aggregator reference
...
contract PriceFeed is Ownable, BaseMath {
    ...
    IBTCOracle public mainOracle; // Main Oracle aggregator
    IBTCOracle public backupOracle; // Backup Oracle
    ...
    function setAddresses(
        ...
        mainOracle = IBTCOracle(_mainOracleAddress);
        backupOracle = IBTCOracle(_backupOracleAddress);
        ...
        Response memory prevMainOracleResponse = _getPrevOracleResponse(mainOracleResponse.roundId);
    ...
    function fetchPrice() external returns (uint256) {
        ...
        if (status == Status.mainOracleWorking) {
            ...
            if (_mainOracleIsBroken(mainOracleResponse, prevMainOracleResponse)) {
            ...
            if (_priceChangeAboveMax(mainOracleResponse, prevMainOracleResponse, decimals)) {
        ...
        if (status == Status.usingBackupMainUntrusted) {
            ...
            if (
                _priceChangeAboveMax(backupOracleResponse, prevBackupOracleResponse, backupDecimals)
            ) {
                _changeStatus(Status.bothOraclesUntrusted);
                return lastGoodPrice;
            }
    ...
    function _mainOracleIsBroken(
        ...
        return _badMainOracleResponse(_currentResponse) || _badMainOracleResponse(_prevResponse);
    }

    function _badMainOracleResponse(Response memory _response) internal view returns (bool) {
        // Check for an invalid roundId that is 0
        if (_response.roundId == 0) {
            return true;
        }
        // Check for an invalid timeStamp that is 0, or in the future
        if (_response.blockNumber == 0 || _response.blockNumber > block.number) {
            return true;
        }
    ...
    function _mainOracleIsFrozen(Response memory _response) internal view returns (bool) {
    ...
    function _priceChangeAboveMax(
        ...
        uint currentScaledPrice = _scalePriceByDigits(uint256(_currentResponse.answer), _decimals);
        uint prevScaledPrice = _scalePriceByDigits(uint256(_prevResponse.answer), _decimals);
        ...
        uint threshold = (maxPrice * MAX_PRICE_DEVIATION_PERCENTAGE_FROM_PREVIOUS_ROUND) / 1000; // 17.5% of max price

        // Return true if price has more than doubled, or more than halved.
        return difference > threshold;
    }

    function _backupOracleIsBroken(Response memory _response) internal view returns (bool) {
        // Check for an invalid roundId that is 0
        if (_response.roundId == 0) {
            return true;
        }
    ...
    function _backupIsFrozen(Response memory _backupResponse) internal view returns (bool) {
    ...
    function _bothOraclesSimilarPrice(
        ...
        uint minPrice = LiquityMath._min(scaledBackupPrice, scaledMainOraclePrice);
        uint maxPrice = LiquityMath._max(scaledBackupPrice, scaledMainOraclePrice);
    ...
    function _scalePriceByDigits(uint _price, uint _answerDigits) internal pure returns (uint) {
        /*
    ...
    function _storeGoodPrice(Response memory _response, uint8 _decimals) internal returns (uint) {
        uint scaledPrice = _scalePriceByDigits(uint256(_response.answer), _decimals);
    ...
    function _getCurrentMainOracleResponse() internal view returns (Response memory response) {
    ...
    function _getCurrentBackupResponse() internal view returns (Response memory response) {
        // Try to get latest price data:
